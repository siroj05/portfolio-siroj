        ...
        {data?.data.length ? data?.data?.map((msg, idx) => {
          return (
            <Link
              ...
              href={`?msg=${msg.id}`}
              className={`p-2 hover:bg-zinc-800 border-t ${data?.data.length -1 == idx && "border-b"} border-zinc-800 ${msg.isRead ? "" : "bg-muted"} ${currentMessage == msg.id && "bg-zinc-800"} flex justify-between`}
            >
