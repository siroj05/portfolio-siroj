...
export default function DynamicTechForm() {
    ...
    const { data: categories, isLoading, isError, isSuccess, isFetching } = useGetAllCategories()
    ...
    return (
        <>
                    ...
                    categories?.data.map((item, i: number) => (
                        <div className="relative" key={item.id}>
                            <Trash2 onClick={() => {
                                setGetId(item.id!)
                                setOpenAlert(true)
                            }} className="absolute -top-2 cursor-pointer right-2 text-red-500 hover:scale-105"/>
                            <Cards
                                key={i}
            ...
            <AlertDialogDelete
                ...
                footer={
                    <>
                        <Button className="cursor-pointer" onClick={() => setOpenAlert(false)}>
                            Cancel
                        </Button>
                        <Button
